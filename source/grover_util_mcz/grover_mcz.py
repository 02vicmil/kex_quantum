...
from .Uf_mcz import Uf_mcz_oracle
from .Uf0_mcz import Uf0_mcz_oracle
...
def grovers_circuit(marked: int, num_qubits: int, iteration_num: int = -1) -> QuantumCircuit:
    ...
    circuit = QuantumCircuit(in_reg, classical_reg)
    ...
    for _ in range(iteration_num):
        # Apply Phase Oracle
        circuit.append(Uf_z, in_reg)

        # Apply Diffusor/Mean Inversion
        circuit.h(in_reg)
        circuit.append(Uf0_z, in_reg)
        circuit.h(in_reg)
